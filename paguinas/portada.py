...
with open('datos_eldeber.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
    # writer.writerow(['paguina','Tipo de noticia', 'Descripción', 'Tiempo', 'Periodista', 'Exclusivo para suscriptores'])
    for i in range(len(divs)):
        if tiempos[i] != "No encontrado":
            writer.writerow(['portada',tipos_noticia[i], descripciones[i], tiempos[i], periodistas[i], suscriptores[i]])
