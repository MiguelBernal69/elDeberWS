...
URL_BASE = "https://eldeber.com.bo/paraellas"
...
with open('datos_eldeber1.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
    # Escribe los nuevos datos en el archivo
    for i in range(len(divs)):
        if tiempos[i] != "No encontrado":
            writer.writerow(['para_ellas',tipos_noticia[i], descripciones[i], tiempos[i], periodistas[i], suscriptores[i]])
